...
\definecolor{darkmagenta}{rgb}{0.55, 0.0, 0.55}% color de las palabras reservadas de base de datos
\definecolor{darkorchid}{rgb}{0.6, 0.2, 0.8}% color del igual
...
\newtheorem{thm}[theo]{Teorema}
\newtheorem{lemma}[theo]{Lemma}
\newtheorem{proposition}[theo]{Proposition}
\newtheorem{corollary}[theo]{Corollary}
\theoremstyle{definition}
\newtheorem{defi}[theo]{Definición}
\newtheorem{exa}[theo]{Ejemplo}
\newtheorem{xca}[theo]{Ejercicio}
\newtheorem{conclusion}[theo]{Conclusion}
\newtheorem{conj}[theo]{Conjectura}
\newtheorem{crit}[theo]{Criterio}
\newtheorem{summary}[theo]{Summary}
\newtheorem{axi}[theo]{Axioma}
\newtheorem{problem}[theo]{Problema}
\numberwithin{algorithm}{section}
\theoremstyle{remark}
\newtheorem{rmk}[theo]{Observación}
...
\begin{document}
    ...
    %Los técnicos alertaron que ``los componentes del programa (que se utiliza para la conversión de formato de los telegramas transmitidos desde las escuelas) tienen al menos 46 vulnerabilidades y pueden ser potencialmente explotadas para dañar la integridad del proceso de escrutinio provisorio''.

    
    %El documento había sido publicado por la Fundación Vía Libre a partir de un trabajo elaborado por el Equipo de Investigación para Perfil.com y las dudas giraban en torno a si el software usado para el escrutinio provisorio modifica los telegramas una vez transmitidos. Los propios técnicos de la empresa contratada por el Estado reconocieron que el scanner genera un archivo TIFF, pero los operadores de carga reciben un PDF. En términos más simples: la imagen que reciben los data entry (que están en el centro de cómputos) no es la misma que sale de la escuela.
